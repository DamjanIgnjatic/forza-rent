...
class Currency implements \IWPML_Backend_Action, \IWPML_Frontend_Action {
	...
	private function getRawPrice( $price ) {
		...
		$defaultGFCurrency = \get_option( 'rg_gforms_currency' ); // \GFCommon::get_currency() - we need currency without WCML modification
		$numberGFPrice     = \GFCommon::to_number( $price, $defaultGFCurrency );

		if ( false !== $numberGFPrice ) {
			return $numberGFPrice;
		}
