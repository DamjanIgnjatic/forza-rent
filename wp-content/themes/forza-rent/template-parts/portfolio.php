<?php if (array_key_exists('item', $args) && $item = $args['item']): ?>
    <div class="slider-item col-12 item-<?php echo $item->ID; ?> portfolio">
        ...
        <a href="<?php echo get_the_permalink($item->ID); ?>" class="btn btn-silent" title="<?php echo get_the_title($item->ID); ?>" target="_blank"><?php _e('Check', 'forzarent'); ?> <?php echo get_the_title($item->ID); ?></a>
        <?php if ($editLink = get_edit_post_link($item->ID)): ?>
            <a href="<?php echo $editLink; ?>" target="_blank"><?php echo __('Edit', 'forzarent'); ?></a>
        <?php endif; ?>
    </div>
<?php endif; ?>